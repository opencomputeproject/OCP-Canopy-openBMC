...
# Load platform-specific DTB overlay based on server ID.
# Reads the server ID from the GXP XREG hardware register and loads
# the corresponding device tree overlay for the platform variant.
load_gxp_dtb_overlay() {
	SERVER_ID_PATH="/sys/devices/platform/ahb@80000000/d1000000.xreg/soc/xreg/server_id"
	CONFIGFS_PATH="/sys/kernel/config"
	OVERLAY_TARGET="${CONFIGFS_PATH}/device-tree/overlays/platform"

	if ! mountpoint -q "${CONFIGFS_PATH}"; then
		mount -t configfs none "${CONFIGFS_PATH}"
	fi

	if [ ! -d "${CONFIGFS_PATH}/device-tree/overlays" ]; then
		echo "Device tree overlay support not available in kernel"
		return 0
	fi

	if [ ! -f "${SERVER_ID_PATH}" ]; then
		echo "Server ID sysfs path not found: ${SERVER_ID_PATH}"
		return 0
	fi

	SERVER_ID=$(cat "${SERVER_ID_PATH}")
	OVERLAY_FILE="/boot/proliant_${SERVER_ID}.dtbo"

	if [ -f "${OVERLAY_FILE}" ]; then
		echo "Loading device tree overlay for server ID ${SERVER_ID}"
		mkdir -p "${OVERLAY_TARGET}"
		if cat "${OVERLAY_FILE}" > "${OVERLAY_TARGET}/dtbo"; then
			echo "Device tree overlay loaded successfully"
		else
			echo "Error: Failed to load device tree overlay"
			rmdir "${OVERLAY_TARGET}" 2>/dev/null
		fi
	else
		echo "No overlay found for server ID ${SERVER_ID} (${OVERLAY_FILE})"
	fi
}
